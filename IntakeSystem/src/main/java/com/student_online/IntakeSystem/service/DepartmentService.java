...
public class DepartmentService {
    ...
    public ResponseEntity<Result> createDepartment(@NotNull Department department) {
        String name=department.getName();
        if(departmentMapper.getDepartmentByName(name)!=null)return ResponseUtil.build(Result.error(409,"该部门已存在"));
        else{
    ...
    public ResponseEntity<Result> updateDepartment(@NotNull Department department) {
        departmentMapper.updateDepartment(department);
        return ResponseUtil.build(Result.ok());
    }

    public ResponseEntity<Result> deleteDepartment(int departmentId) {
        try {
    ...
    public ResponseEntity<Result> getStation(@NotNull Department department) {//返回部门的上一级模块
